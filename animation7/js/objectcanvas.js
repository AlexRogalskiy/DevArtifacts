/* FusiУГn del ObjetoCanvas (originalmente destinado a tests de banners) con el ObjetoBanner 
    Creado el 14/10/2016 por Josep Antoni Bover Comas para devildrey33.es 
    Ultima modificaciУГn :  30/08/2017 
 */
 ...
 * La estructura HTML puede que sea excesiva, para una demostraciУГn 2d/3d pero ofrece ciertas ventajas, de las que en teorУ­a ya no me tengo que preocupar, por ejemplo :
 *  Muestra los frames por segundo si no se desactiva en las opciones 
 *  Muestra si se estУЁ cargando algun archivo externo (texturas/modelos/audio/video/etc..) basta con utilizar ObjetoCanvas.Cargando(BOOL) 
 *  Permite pausar el canvas si este se encuentra en segundo plano, y nos muestra un mensaje si estУЁ pausado.
 *  (SOLO EN Entorno:'Normal') Muestra un boton iniciar animaciУГn, que ademУЁs pone el canvas a pantalla completa. */
...
/*  Opciones['Tipo']                    puede ser : 
    ...
    Opciones['Entorno']                 puede ser :
                                            ...
                                            - Banner   , Canvas diseУБado para el Banner de devildrey33.
    Opciones['Idioma']                  puede ser : 'es'paУБol o 'en'glish.                              ('es' POR DEFECTO)
    ...
    Opciones['ElementoRaiz']            elemento del HTML donde se crearУЁ el canvas                     (POR DEFECTO es 'document.body')
    Opciones['ColorFondo']              color del fondo en HEX (SOLO para THREE.js)                     (POR DEFECTO es '0x312E35' gris oscuro) 
    Opciones['CapturaEjemplo']          nombre del archivo que contiene la captura de pantalla          (EnlazarУЁ a '/Web/Graficos/250x200_')
...
var ObjetoCanvas = function(Opciones) {
    ...
    this.OpcionesCanvas  = { 
        ...
        Pausar                  : true,             // Pausar si el canvas estУЁ en segundo plano
    ...
    if (this.OpcionesCanvas['Entorno'] === 'Normal') {
        ...
        // Creo las etiquetas que contienen informaciУГn adicional sobre la animaciУГn
        ...
        this.Textos = { 
            ...
            es : ["Cargando...", "En Pausa", "Iniciar", "Frames por segundo", "Pantalla Completa", "Restaurar Pantalla", "PУЁgina de devildrey33", "Error iniciando WebGL" ] 
        };
...
//            else                                                    { bPantallaCompleta.style.display = "none"; }
            ...
            // Eventos para determinar si estУЁ en pantalla completa
...
//        if (this.OpcionesCanvas.BotonLogo === false) { document.getElementById("ObjetoCavas_Logo").style.display = "none"; }                       
    ...
    // En el entorno Banner las etiquetas ya estan creadas, pero hay que eliminar y volver a crear el canvas
        ...
        // Marco que contiene la informaciУГn de la animaciУГn del banner
...
//    this._EsMovil = -1; // no se ha hecho la detecciУГn de dispositivos moviles
    ...
    // Si el canvas es de ancho fijo, aУБado el css para centrar-lo
...
// FunciУГn que devuelve el pixel ratio del dispositivo actual
...
// FunciУГn que determina el estado de carga (cargando/completo) true/false
ObjetoCanvas.prototype.Cargando = function(carga) {
    this.Cabecera.setAttribute("cargando", carga);
};

// FunciУГn que obtiene el estado de carga
ObjetoCanvas.prototype.EstaCargando = function() {
    var Ret = this.Cabecera.getAttribute("cargando");
    return (Ret === "true" || Ret === true);
};

// FunciУГn interna utilizada por requestAnimationFrame para actualizar y pintar la animaciУГn
...
// FunciУГn que procesa el evento keydown
...
// FunciУГn que procesa el evento keyup
...
// FunciУГn que procesa el evento mouseup
...
// FunciУГn que procesa el evento touchstart
...
// FunciУГn para pausar la animaciУГn
// - Hay que detectar cuando la animaciУГn no es visible y cuando la ventana no tiene el foco para pausar la animaciУГn
// - En modo depuraciУГn nunca se hace la pausa (esto es para poder depurar el Three.js en el Three.js.inspector)
...
// FunciУГn para reanudar la animaciУГn desde el ultimo punto
...
// FunciУГn que controla el scroll para determinar si la animaciУГn sigue a la vista o no y de esta forma detenerla / reanudarla
ObjetoCanvas.prototype.EventoScroll = function() {
    // Llamo a la funciУГn Scroll del NuevoObjeto (si existe)
    ...
    // Llamo a la funciУГn Scroll si existe en la clase heredada.
    if (typeof(this.Scroll) !== "undefined") {
        this.Scroll.apply(this);
    }            
};

// FunciУГn que cuenta los frames por segundo
...
ObjetoCanvas.prototype.EventoPantallaCompleta = function(Evento) {    
    ...
    // Si no encuentro la funciУГn fullscreenElement (en firefox no va...), miro si la altura de la pantalla es igual a la altura de la pestaУБa
    ...
    // Si screenTop es 0 es que estУЁ en modo pantalla completa.
...
/* FunciУГn para generar un valor aleatУГrio entero */
/* Si no se especifican parametros devuelve 0 o 1 */
/* Si solo se especifica un parУЁmetro, el primer parУЁmetro serУЁ el mУЁximo, y el mУ­nimo serУЁ 0 */
/* Si se especifican dos parУЁmetros, el primero es el mУЁximo, y el segundo es el mУ­nimo. */
...
/* Si no se especifican parametros devuelve 0 o 1 
   Si se especifica solo el MУЁximo, el mУ­nimo serУЁ 0 */
function Rand(Max, Min) {
    var min = (typeof(Min) !== "undefined") ? Min : 0; // Si no se especifica el mУ­nimo por defecto es 0
    var max = (typeof(Max) !== "undefined") ? Max : 1; // Si no se especifica el mУЁximo por defecto es 1
