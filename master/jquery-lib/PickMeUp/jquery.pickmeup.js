...
(function (factory) {
    ...
    var views = {
        ...
        tpl = {
            ...
            body: function (elements, container_class_name, render_callback) {
                var options = $(this).data('pickmeup-options');
                var result = '';
                for (var i = 0; i < elements.length; ++i) {
                    if (render_callback) {
                        render_callback.apply(this, [elements[i]]);
                    }
                    result += '<div class="' + elements[i].class_name + ' pmu-button" data-date="'+elements[i].date+'">' + elements[i].text + '</div>'
                }
    ...
    function fill() {
            ...
            (function () {
                ...
                for (var j = 0; j < 42; ++j) {
                    day = {
                        ...
                        date: formatDate(local_date, options.format),
                        class_name: []
                    };
                ...
                html += tpl.body(days, 'pmu-days', options.render_cell);
            })();
        ...
        if (options.min && options.min >= shown_date_from) {
            pickmeup.find('.pmu-prev').addClass('disabled');
        } else {
            pickmeup.find('.pmu-prev').removeClass('disabled');
        }

        if (options.max && options.max <= shown_date_to) {
            pickmeup.find('.pmu-next').addClass('disabled');
        } else {
            pickmeup.find('.pmu-next').removeClass('disabled');
        }
    ...
    function show(force) {
        ...
        if (force || !pickmeup.is(':visible')) {
            ...
            if (!options.flat) {
                ...
                $(document)
                    .on(
                    'mousedown' + options.events_namespace + ' ' + options.events_namespace,
                    options.binded.hide
                )
                    .on(
