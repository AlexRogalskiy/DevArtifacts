...
export function createCounterRangeValidator(maxValue, minValue) {
	const DEFAULT_MAX_VALUE: number = 9999;
	const DEFAULT_MIN_VALUE: number = 1;
	...
	`
		<span [formGroup]="ticketForm" class="ticket-form" novalidate>
			...
			<span class="counter-value">
				<input type="text" class="ticketCounter" formControlName="ticketCounter" [(ngModel)]="item" min="1" max="9999" (change)="changeCounter($event)" [min]="1" [max]="9999" step="1" minlength="1" maxlength="4" pattern="\d{1,4}" value="{{ counterValue }}" number required />
			</span>
...
export class TicketCounter implements ControlValueAccessor, OnChanges, OnInit {
	...
	@Input('counterRangeMin') counterMin = 1;
	@Input('counterRangeMax') counterMax = 9999;
	...
	public ngOnChanges(inputs) : void {
		if (inputs.counterMax || inputs.counterMin) {
			this.validateFn = createCounterRangeValidator(this.counterMax, this.counterMin);
