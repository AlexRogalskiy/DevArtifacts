    ...
    <modal #shoppingCart [animation]="animation" [keyboard]="keyboard" [backdrop]="backdrop" (onClose)="closed()" (onDismiss)="dismissed()" (onOpen)="opened()" [cssClass]="cssClass" #validationModal>
        <form #modalForm="ngForm" novalidate>
            ...
            <modal-footer>
				...
				<button type="button" class="btn btn-default" data-dismiss="modal" (click)="saveShoppingCart($event); validationModal.close();">Сохранить</button>
                <button type="button" class="btn btn-primary" [disabled]="!modalForm.valid" (click)="buyShoppingCart($event)">Купить</button>
            </modal-footer>
        </form>
    </modal>
	`,
