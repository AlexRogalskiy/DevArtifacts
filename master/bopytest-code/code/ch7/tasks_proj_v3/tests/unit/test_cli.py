...
def test_list_no_args(mocker):
    ...
    mocker.patch.object(tasks.cli.tasks, 'list', return_value=[])
    runner = CliRunner()
    runner.invoke(tasks.cli.tasks_cli, ['list'])
    tasks.cli.tasks.list.assert_called_once_with(None)
...
def test_list_print_many_items(no_db, mocker):
    ...
    mocker.patch.object(tasks.cli.tasks, 'list', 
...
def test_list_dash_o(no_db, mocker):
    ...
    tasks.cli.tasks.list.assert_called_once_with('brian')
...
def test_list_dash_dash_owner(no_db, mocker):
    ...
    tasks.cli.tasks.list.assert_called_once_with('okken')
