...
const getFormatter = (options, locales = getLocale()) => {
  ...
  if (!formatterCache[locales][stringifiedOptions]) {
    formatterCache[locales][stringifiedOptions] = new Intl.DateTimeFormat(
      locales,
      options
    ).format
  }
...
export const formatDate = date =>
  getFormatter({ day: 'numeric', month: 'numeric', year: 'numeric' })(
    toSafeHour(date)
  )

export const formatMonthYear = date =>
  getFormatter({ month: 'long', year: 'numeric' })(toSafeHour(date))

export const formatMonth = date =>
  getFormatter({ month: 'long' })(toSafeHour(date))

export const formatShortWeekday = date =>
  getFormatter({ weekday: 'short' })(toSafeHour(date))
