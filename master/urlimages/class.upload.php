...
var $referrer;
     ...
     */
    function upload($file, $from_url, $lang = 'en_GB') {
        ...
        if (!is_array($file)) {
            if (empty($file)) {
				...
				if($from_url){
					// this is a url
					$this->log .= '<b>' . $this->translate("source is a url image") . ' ' . $file . '</b><br />';
				}
				else{
                ...
                if ($this->uploaded && !file_exists($file) && !$from_url) {
                    $this->uploaded = false;
                    $this->error = $this->translate('local_file_missing');
                }

                if ($this->uploaded && !is_readable($file) && !$from_url) {
                    $this->uploaded = false;
     ...
     */
    function process($server_path = null,$from_url = false) {
        ...
        // otherwise, if we don't check on uploaded files (local file for instance), we use file_exists()
            ...
            if (!file_exists($this->file_src_pathname) && !$from_url) {
                $this->processed = false;
                $this->error = $this->translate('source_missing');
            }
        }
...
function test($referrer){
								        ...
								        if ($isError){
								            ...
								            $_SESSION['imgerror'] = "1";
								            if($referrer == 'design.php'){
								            	$_SESSION['backgroundpanelpos'] = "1";
								            if(isset($_POST['colorspanelpos'])){
												if($_POST['colorspanelpos']=='1'){
													$_SESSION['colorspanelpos'] 	= "1";		
												}	
												$_SESSION['_curr_link_color'] 		= $_POST['_curr_link_color'];
												$_SESSION['_curr_background_color'] = $_POST['_curr_background_color'];
												$_SESSION['_curr_background_image'] = $_POST['_curr_background_image'];
												$_SESSION['_default_css'] 			= $_POST['_default_css'];
												$_SESSION['uploaded_image'] 		= $_POST['uploaded_image'];
												$_SESSION['repeat2']         		= $_POST['repeat2'];
											}
								            }
								            //echo "Script execution halted ({$error['message']})";
